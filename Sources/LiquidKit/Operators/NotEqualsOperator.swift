...
/// 
/// The not-equals operator performs strict inequality comparison between two values. It is commonly
/// used in conditional tags like `if`, `unless`, and `case` to control template flow based on whether
/// values do not match. The operator returns `true` when values are different and `false` when they
/// are the same, using the exact opposite logic of the `==` operator.
/// 
/// Like the equality operator, the not-equals operator is strict about types and does not perform
/// type coercion. This means the string `"1"` is not equal to the integer `1` (so `!=` returns true),
/// and the integer `0` is not equal to the boolean `false` (so `!=` returns true). This strict
/// comparison ensures predictable template behavior and helps catch type mismatches early.
/// 
...
/// ```
/// 
/// Type-strict comparisons showing inequality:
/// ```liquid
/// {% if 1 != "1" %}true{% else %}false{% endif %}
/// Output: true (different types)
/// 
/// {% if 0 != false %}true{% else %}false{% endif %}
/// Output: true (number vs boolean)
/// 
/// {% if 1.0 != 1 %}true{% else %}false{% endif %}
/// Output: false (numeric types with same value)
/// ```
/// 
...
///   holds for all value types including arrays, dictionaries, and special values like nil.
/// 
