...
@Suite(.tags(.filter, .plusFilter))
struct PlusFilterTests {
    private let filter = PlusFilter()
    
    // MARK: - Basic Arithmetic
    
    @Test("Integer addition")
    func integerAddition() throws {
        try validateEvaluation(of: 10, with: [.integer(2)], by: filter, yields: 12)
        try validateEvaluation(of: 5, with: [.integer(10)], by: filter, yields: 15)
        try validateEvaluation(of: 100, with: [.integer(50)], by: filter, yields: 150)
    ...
    func decimalAddition() throws {
        try validateEvaluation(of: 10.1, with: [.decimal(2.2)], by: filter, yields: Token.Value.decimal(Decimal(string: "12.3")!))
        try validateEvaluation(of: 183.357, with: [.decimal(12)], by: filter, yields: Token.Value.decimal(Decimal(string: "195.357")!))
        try validateEvaluation(of: 5.5, with: [.decimal(10.5)], by: filter, yields: Token.Value.decimal(Decimal(16.0)))
    ...
    @Test("Mixed integer and decimal addition")
    func mixedAddition() throws {
        // Integer plus decimal returns decimal
        try validateEvaluation(of: 10, with: [.decimal(2.0)], by: filter, yields: Token.Value.decimal(Decimal(12.0)))
        try validateEvaluation(of: 10, with: [.decimal(2.5)], by: filter, yields: Token.Value.decimal(Decimal(12.5)))
        
        // Decimal plus integer returns decimal
        try validateEvaluation(of: 10.0, with: [.integer(2)], by: filter, yields: Token.Value.decimal(Decimal(12.0)))
        try validateEvaluation(of: 10.5, with: [.integer(2)], by: filter, yields: Token.Value.decimal(Decimal(12.5)))
    }
    ...
    func stringConversion() throws {
        ...
        try validateEvaluation(of: "10", with: [.string("2")], by: filter, yields: 12)
        try validateEvaluation(of: "10.1", with: [.string("2.2")], by: filter, yields: Token.Value.decimal(Decimal(string: "12.3")!))
        try validateEvaluation(of: "16", with: [.integer(4)], by: filter, yields: 20)
        
        // Negative string numbers
        try validateEvaluation(of: "-10", with: [.string("5")], by: filter, yields: -5)
        try validateEvaluation(of: "10", with: [.string("-5")], by: filter, yields: 5)
        
        // Whitespace handling
        try validateEvaluation(of: " 10 ", with: [.string(" 2 ")], by: filter, yields: 12)
        try validateEvaluation(of: "\t10.5\n", with: [.string("0.5")], by: filter, yields: Token.Value.decimal(Decimal(11.0)))
    }
    ...
    func integerTypePreservation() throws {
        // Integer plus integer with whole result preserves integer type
        try validateEvaluation(of: 10, with: [.integer(2)], by: filter, yields: Token.Value.integer(12))
        try validateEvaluation(of: -5, with: [.integer(-3)], by: filter, yields: Token.Value.integer(-8))
        
        // Even when result could be represented as integer, decimal operands return decimal
        try validateEvaluation(of: 10.0, with: [.decimal(2.0)], by: filter, yields: Token.Value.decimal(12.0))
    }
    ...
    func scientificNotation() throws {
        try validateEvaluation(of: "1e3", with: [.string("2e2")], by: filter, yields: 1200)
        try validateEvaluation(of: "5.5e1", with: [.string("5e0")], by: filter, yields: Token.Value.decimal(Decimal(60)))
        try validateEvaluation(of: "-1e2", with: [.string("1e1")], by: filter, yields: -90)
    }
    ...
    func extraParameters() throws {
        ...
        try validateEvaluation(of: 10, with: [.integer(3), .integer(5), .integer(7)], by: filter, yields: 13)
        try validateEvaluation(of: 10.5, with: [.decimal(0.5), .string("ignored")], by: filter, yields: Token.Value.decimal(Decimal(11.0)))
    }
    ...
    func documentationExamples() throws {
        // From LiquidJS docs
        try validateEvaluation(of: 4, with: [.integer(2)], by: filter, yields: 6)
        try validateEvaluation(of: 16, with: [.integer(4)], by: filter, yields: 20)
        try validateEvaluation(of: 183.357, with: [.integer(12)], by: filter, yields: Token.Value.decimal(Decimal(string: "195.357")!))
        
        // From Python Liquid docs
        try validateEvaluation(of: "16", with: [.string("4")], by: filter, yields: 20)
        try validateEvaluation(of: 183.357, with: [.decimal(12)], by: filter, yields: Token.Value.decimal(Decimal(string: "195.357")!))
        try validateEvaluation(of: "hello", with: [.integer(10)], by: filter, yields: 10)
    }
}